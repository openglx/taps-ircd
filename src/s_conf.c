 ...
 *
 *  $Id: s_conf.c 34 2010-07-31 18:35:02Z openglx $
 */
#include "s_conf.h"
#include "channel.h"
#include "class.h"
...
void iphash_stats(aClient *cptr, aClient *sptr,int parc, char *parv[],int out)
{
  ...
  if(out < 0)
  ...
  else
    {
      ...
      if (-1 == write(out,result_buf,strlen(result_buf)))
        {
...
{
  struct ConfItem *aconf = match_Dline((uint32_t)ntohl((unsigned long)addr.S_ADDR));
