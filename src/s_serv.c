 ...
 *
 *   $Id: s_serv.c 34 2010-07-31 18:35:02Z openglx $
 */
#include "s_serv.h"
#include "msg.h"		
#include "channel.h"
  ...
  if (IsPerson(acptr))
    {
      ...
      if (!*ubuf)
        { /* trivial optimization - Dianora */
          ...
          if (!inet_ntop(AF_INET, (void*)&(acptr->ip),
              &ip_addr, sizeof(ip_addr)))
          {
              snprintf(ip_addr, INET_ADDRSTRLEN, "255.255.255.255");
              sendto_ops("addrstrlen %s", strerror(errno));
          }

	  if (acptr->user->vlink)
	    sendto_one(cptr, "NNICK %s %d %lu %s %s %s %s %s!%s :%s",
	    		acptr->name, acptr->hopcount+1, acptr->tsinfo, ubuf,
	    		acptr->username, ip_addr, acptr->host,
