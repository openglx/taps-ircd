 ...
 */
int
m_svsmode (aClient * cptr, aClient * sptr, int parc, char *parv[])
{
  aClient *acptr;
  int what, setflags;
  int flag;
  ...
  if (!IsServer (cptr) || !IsService (sptr))
    {
      if (IsServer (cptr))
	{
	  ts_warn ("Got SVSMODE from non-service: %s", sptr->name);
	  sendto_one (cptr,
		      ":%s WALLOPS :ignoring SVSMODE from non-service %s",
		      me.name, sptr->name);
	}
      return 0;
    }
  ...
  if ((acptr = find_person (parv[1], NULL)) && MyClient (acptr))	/* person found connected here */
	...
	switch (*m)
	  ...
	  case 'n':
	    if (extrapar > 0 && acptr->user)
	      {
		acptr->user->news_mask =
		  strtoul (parv[3 + extrapar], NULL, 0);
		--extrapar;
	      }
	    break;
	  case 'l':
		...
		if (MyConnect (acptr))
		  {
		    ...
		    acptr->flags2 &= ~(FLAGS2_OPER_GLOBAL_KILL |
				       FLAGS2_OPER_REMOTE |
				       FLAGS2_OPER_UNKLINE |
				       FLAGS2_OPER_GLINE | FLAGS2_OPER_K);
		    while (cur_cptr)
		      {
			if (acptr == cur_cptr)
			  {
			    if (prev_cptr)
			      prev_cptr->next_oper_client =
				cur_cptr->next_oper_client;
			    else
	...
	strncpy (newmask, SAdminMask, HOSTLEN + USERLEN + 2);
      else if ((IsHelper (acptr)) && HelperMask
	       && (!IsOper (acptr) || !OperMask))
