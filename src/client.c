 ...
 *
 *  $Id: client.c 34 2010-07-31 18:35:02Z openglx $
 */
#include "client.h"
#include "class.h"
#include "channel.h"
      ...
      if (rehashed)
        {
          if(dline_in_progress)
            {
#ifdef IPV6
              if( (aconf = match_Dline((uint32_t)ntohl((uint32_t)(cptr->ip.S_ADDR)))) )
#else
              if( (aconf = match_Dline((uint32_t)ntohl(cptr->ip.s_addr))) )
#endif
            ...
            if (IsPerson(sptr))
              {
                ...
                ircsprintf(linebuf,
                           "%s (%3d:%02d:%02d): %s!%s@%s %d/%d\n",
                            ...
                            sptr->sendK,
                            sptr->receiveK);
                if (-1 == write(logfile, linebuf, strlen(linebuf)))
                  {
		    irclog(L_CRIT, "Failed to write on logfile FD %d: '%s'",
                      logfile, linebuf);
                  }
                /*
