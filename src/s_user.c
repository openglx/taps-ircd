 ...
 *
 *  $Id: s_user.c 33 2010-05-10 02:46:01Z openglx $
 */
#include "s_user.h"
  ...
  if (MyConnect(sptr))
	...
	/*       a purely cosmetical change */
      ...
      if(!valid_hostname(sptr->host))
        {
          sendto_one(sptr,":%s NOTICE %s :*** Notice -- You have an illegal character in your hostname (%s)", 
                     mename, sptr->name, sptr->host );
